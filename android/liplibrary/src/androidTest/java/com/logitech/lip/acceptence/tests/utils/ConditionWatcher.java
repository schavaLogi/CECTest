...
public class ConditionWatcher {
    ...
    public static final int DEFAULT_MAX_TIMEOUT_LIMIT = 1000 * 60;
    public static final int DEFAULT_MIN_TIMEOUT_LIMIT = 1000 * 10;

    public static final int DEFAULT_INTERVAL = 250;

    private int timeoutLimit = DEFAULT_MAX_TIMEOUT_LIMIT;
    ...
    public static void waitForCondition(Instruction instruction) throws Exception {
        ...
        if (status == TIMEOUT) {
            throw new Exception(instruction.getDescription() + " - took more than "
                    + getInstance().timeoutLimit / 1000 + " seconds. Test stopped.");
        }
    }
