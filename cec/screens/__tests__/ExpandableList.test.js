...
// UI Dependency
import {Button, Container, Icon, Text, Title} from "native-base";
...
const props = {
    navigation : {navigate : jest.fn(), state : {} , dispatch : jest.fn() , goBack: jest.fn()},
    dispatch : jest.fn(),
    cecDevices : CEC_DEVICES
};

const defaultState = {activeSection: false, collapsed: true};
...
describe('ExpandableListScreen Testing', () => {
    ...
    it('Check we are able to click on Header so that We can render Content', ()=>{
        ...
        expect(wrapper.state()).toEqual({activeSection : 3, collapsed: true});
